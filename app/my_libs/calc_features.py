...
def mean_chem(df):
    mean = pd.read_csv('app/DATA/prepared/mean_chem_steel.csv')
...
>>>>>>> f308dd357f9e616f12cb69b79c15d41170bc56b4
    ...
    input_filename = os.getcwd() + '/app/input/' + "prediction_input_" + time + "_" + username + ".xlsx"
    df.to_excel(input_filename)
    ...
    try:
        ...
        try:
            ...
            ls_need_col.remove('Дата термообработки')
            df = df[ls_need_col]
            ls_need_col = ls_need_col + ['Дата термообработки']
...
def calc_quenching_param(df):
    ...
    df['Параметр закалка'] = 1 / (1 / (df[['3 зона по ВТР закалка']].mean(axis=1)
                                       + 273) - (2.303 * 1.986 * np.log10(
        61 * df['шаг балок закалочная печь, сек'].astype(float) / 3600) / 110000)) - 273
    return df


def calc_tempering_param(df):
    """Считает параметр отпуска, возвращает датафрейм с добавленным признаком"""
    df['Параметр отпуск'] = (df[['3 зона ВТР и уставка отпуск', '4 зона ВТР и уставка отпуск',
                                 '5 зона ВТР и уставка отпуск']].mean(axis=1)
                             + 273) * (20 + np.log(
        94 * df['шаг балок отпускная печь, сек'].astype(float) / 3600)) * 1e-3
    return df
...
def calc_tempering_param_new(df):
    ...
    df['Параметр отпуск новый'] = (np.log(94 * df['шаг балок отпускная печь, сек'].astype(float) / 3600) - (
            (114.7 * (df['Mo'] + df['Mn'] / 5 + df['Cr'] / 10) + 46.6) / 4.6 / (
            df[['2 зона ВТР и уставка отпуск', '3 зона ВТР и уставка отпуск', '4 зона ВТР и уставка отпуск',
                '5 зона ВТР и уставка отпуск']].mean(axis=1)
            + 273)) + 50)
    return df
...
def calc_tempering_param_new_2(df):
    """Считает параметр отпуска, возвращает датафрейм с добавленным признаком"""
    df['Параметр отпуск новый 2'] = (df[['3 зона ВТР и уставка отпуск', '4 зона ВТР и уставка отпуск',
                                         '5 зона ВТР и уставка отпуск']].mean(axis=1)
                                     + 273) * ((
                                                       17.369 - 6.661 * df['C'] - 1.604 * df['Mn'] - 3.412 * df[
                                                   'Si'] - 0.248 * df['Ni'] - 1.112 * df['Cr'] - 4.355 * df['Mo']) +
                                               np.log(150 * df['шаг балок отпускная печь, сек'].astype(
                                                   float) / 3600)) * 1e-3
    return df
...
def calc_tempering_param_new_V(df):
    """Считает параметр отпуска, возвращает датафрейм с добавленным признаком"""
    df['Параметр отпуск новый V'] = (df[['3 зона ВТР и уставка отпуск', '4 зона ВТР и уставка отпуск',
                                         '5 зона ВТР и уставка отпуск']].mean(axis=1)
                                     + 273) * ((
                                                       17.369 - 6.661 * df['C'] - 1.604 * df['Mn'] - 3.412 * df[
                                                   'Si'] - 0.248 * df['Ni'] - 1.112 * df['Cr'] - 4.355 * df[
                                                           'Mo'] - 2.6 * df['V']) +
                                               np.log(94 * df['шаг балок отпускная печь, сек'].astype(
                                                   float) / 3600)) * 1e-3
    return df
...
def calc_seed_size(df):
    ...
    df['Величина зерна'] = np.exp(
        10.313 + 0.93 * df['C'].astype(float) * (1 - 0.469 * df['Cr'].astype(float)) + df['sum_piXi'].astype(float)) * (
                                   np.exp(-(77978 + 3116 * df['C'].astype(float) + df['sum_qiXi'].astype(
                                       float)) / 8.314 / (T + 273)) * (
                                               (t * df['шаг балок закалочная печь, сек']) ** 0.107))
...
def calc_AC1_1(df):
    df[chem] = df[chem].fillna(0)
    df['AC1_1'] = (
                723 - 7.08 * df.Mn + 37.7 * df.Si + 18.1 * df.Cr + 44.2 * df.Mo - 8.95 * df.Ni + 50.1 * df.V + 21.7 * df.Al + 3.18 * df.W + \
...
def calc_AC(df):
    ...
    df = calc_AC1_1(df)
    df = calc_AC1_2(df)
    df['AC3'] = (df['AC3_1'] + df['AC3_2']) / 2.
    df['AC1'] = (df['AC1_1'] + df['AC1_2']) / 2.
    del df['AC3_1']
    del df['AC3_2']
    del df['AC1_1']
    del df['AC1_2']
